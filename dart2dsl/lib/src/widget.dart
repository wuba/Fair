...
Future<ComponentParts?> processFile(AnalysisContext context, String path,
    {bool analysisExports = false,
    bool analysisAllClasses = false,
    Set<String> skips = const <String>{}}) async {
  ...
  if (analysisExports) {
    ...
    var exportsUnits =
        exports.map((e) => e.exportedLibrary?.definingCompilationUnit).toList();
  ...
  for (var element in exposedAPI) {
    ...
    if (element.constructor != null) {
      for (var ctor in element.constructor!) {
        if (ctor.name == null ||
            skips.any((skip) => RegExp(skip).hasMatch(ctor.name!))) {
          continue;
        }
    ...
    if (element.staticMethods != null) {
      for (var method in element.staticMethods!) {
        if (method.name == null ||
            skips.any((skip) => RegExp(skip).hasMatch(method.name!))) {
          continue;
        }
...
Map<String, dynamic> _writeMethod(StringBuffer buffer, String? name,
  ...
  final constString = element is Constructor &&
          (element.parameters == null || element.parameters!.isEmpty) &&
          element.isConst
      ? 'const'
      : '';
    ...
    for (var i = 0; i < element.parameters!.length; i++) {
      ...
      buffer.write(p.isNamed == true
          ...
          : p.isOptionalPositional == true
              ? prop.replaceAll('props[\'${p.name}\']',
                  '(props[\'pa\'].length > $i ? props[\'pa\'][$i]: null)')
              : positionDeclare);
    }
    var params = element.parameters?.fold(
        '',
        (String? value, p) => ((value ?? '') +
            (p.isNamed == true ? '${p.type} ${p.name}, ' : '${p.name}, ')));
    print('➡️ $name({$params})');
  } else {
    print('➡️ $name()');
  }
...
bool _tryInternalCheck(
    InterfaceType type, List<String>? widgets, classElement) {
  // try {
...
class Constructor extends Method {
  Constructor(String name,
      {List<Parameter>? parameters, bool? isWidget, this.isConst = false})
...
class ConstField extends Exposed {
  ...
  ConstField(String name,
      {this.field, this.isWidget = true, this.isStatic = false})
      : super(name);
...
List<ClassExposed> _visit(CompilationUnitElement? unitElement,
    [bool isSdk = false, bool analysisAllClasses = false]) {
  ...
  for (var classElement in apis) {
    if (classElement is ClassElement &&
        _blackList.contains(classElement.name)) {
      continue;
    }
    if ((classElement is ClassElement &&
            classElement.isAbstract &&
            !classElement.fields.any((element) => element.isStatic) &&
            !classElement.methods.any((element) => element.isStatic)) ||
    ...
    var functionParameters = <FunctionParameter>[];
    var isWidget = analysisAllClasses
        ? _isWidget(classElement.thisType)
        : classElement.thisType is InterfaceType
    ...
    if (isWidget ||
        isAPI ||
        isSdk ||
        (analysisAllClasses && !_isState(classElement.thisType))) {
              ...
              constructorElement.parameters.forEach((e) {
                ...
                if (e.type is FunctionType) {
                  ...
                  functionParameters.add(FunctionParameter(
                      functionType.getDisplayString(withNullability: true),
    ...
    for (var fieldElement in classElement.fields) {
      if (!_invalidElement(fieldElement) &&
          (fieldElement.isConst || fieldElement.isStatic)) {
        print('  ❤️${classElement.name}.${fieldElement.name}');
        // 复用class类型（可能不一致）
        fields.add(ConstField(classElement.name,
            field: fieldElement.name,
            isWidget: _isWidget(fieldElement.type),
    ...
    for (var methodElement in classElement.methods) {
      ...
      if (methodElement.parameters.isNotEmpty) {
        parameters = methodElement.parameters.map((e) {
          if (e.type is FunctionType) {
            var functionType = e.type as FunctionType;
            //if(functionType.parameters.isNotEmpty) {
            functionParameters.add(FunctionParameter(
      ...
      staticMethods.add(Method(name,
          parameters: parameters,
          isWidget: _isWidget(methodElement.returnType)));
    }
...
bool _isType(DartType dartType, String type) {
  if (dartType is InterfaceType) {
    return dartType.allSupertypes
        .any((element) => element.element.name == type);
  }
  return false;
}
