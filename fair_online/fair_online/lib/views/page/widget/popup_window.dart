...
///弹窗方法
Future<dynamic> showPopupWindow<T>({
...
class _PopupWindowRoute<T> extends PopupRoute<T> {
  ...
  Widget buildPage(BuildContext context, Animation<double> animation,
    ...
    Widget win = _PopupWindow<T>(
      ...
      fullWidth: fullWidth as bool,
    );
    if (theme != null) {
      win = Theme(data: theme as ThemeData, child: win);
    }
            ...
            child: GestureDetector(
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  ///背景颜色判断
                  color: isShowBg as bool
                      ? Colors.transparent
                      : Colors.transparent,
                  child: CustomSingleChildLayout(
                    delegate: _PopupWindowLayoutDelegate(
                        position as RelativeRect,
                        null,
...
class _PopupWindow<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      animation: route?.animation as Listenable,
      builder: (BuildContext context, Widget? child) {
        return Opacity(
          opacity: opacity.evaluate(route?.animation as Animation<double>),
          child: Material(
            type: route?.elevation == 0
                ? MaterialType.transparency
                : MaterialType.card,
            elevation: route?.elevation as double,
            child: Align(
              alignment: AlignmentDirectional.topEnd,
              widthFactor:
                  width.evaluate(route?.animation as Animation<double>),
              heightFactor:
                  height.evaluate(route?.animation as Animation<double>),
              child: Semantics(
                scopesRoute: true,
                namesRoute: true,
